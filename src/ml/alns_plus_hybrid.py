'''
ALNS+ with hybrid operator selector (Roulette + Neural REINFORCE).
...
Selector options:
  ...
  - 'neural'   : neural network selector trained online via REINFORCE
  - 'hybrid'   : blends roulette and neural probabilities
                 combined_probs = alpha * roulette_probs + (1-alpha) * neural_probs
                 alpha = 0.5 (equal weight)
                 sampling draws from combined distribution,
                 log_prob computed through neural network to preserve gradient flow
                 both roulette weights and neural network update after each iteration
...
class ALNS_plus:
    def __init__(self, inst: CVRPInstance, selector='roulette'):
        ...
        # neural selector networks + optimizers + hyrbid option
        if selector in ('neural', 'hybrid'):
    ...
    def solve(self, initial_solution, n_iter=1000, n_seg=100):
        ...
        for i in range(n_iter):
            ...
            for j in range(n_seg):
                ...
                features = extract_features(current_sol, current_obj, i * n_seg + j, n_iter * n_seg, self.inst) if self.selector in ('neural', 'hybrid') else None
                ...
                if self.selector in ('neural', 'hybrid') and reward > 0:
            ...
            # after each segment
            if self.selector in ('roulette', 'hybrid'):
    ...
    def select_operator(self, op_weights, op_type, features=None):
        if self.selector == 'roulette':
            return self.roulette_wheel(op_weights), None
        elif self.selector == 'neural':
            return self.neural_network(op_weights, op_type, features)
        elif self.selector == 'hybrid': # hybrid caseeeee
            return self.hybrid_select(op_weights, op_type, features)
        else:
            raise ValueError(f"invalid selector type: {self.selector}")


    # adding a hybrid select operator
    def hybrid_select(self, op_weights, op_type, features, alpha=0.5):
        # roulette probabilities — from past scores
        roulette_probs = np.array([w / sum(op_weights) for w in op_weights])

        # neural probabilities — from current state
        with torch.no_grad():
            if op_type == 'destroy':
                logits = self.destroy_net(features)
            else:
                logits = self.repair_net(features)
        neural_probs = torch.softmax(logits, dim=-1).numpy()
        ...
        # sample from the combined distribution (roulette + neural blended),
        # but compute log_prob through the neural network so gradients flow properly.

        combined_tensor = torch.tensor(combined_probs, dtype=torch.float32, requires_grad=False)
        dist     = torch.distributions.Categorical(probs=combined_tensor)
        action   = dist.sample()

        # get log_prob through the neural network to keep gradient tracking
        if op_type == 'destroy':
            logits = self.destroy_net(features)
        else:
            logits = self.repair_net(features)
        neural_dist = torch.distributions.Categorical(logits=logits)
        log_prob    = neural_dist.log_prob(action)  # tracked through network
        action      = action.item()

        # This uses Categorical like the neural selector does ->  properly tracked gradients
