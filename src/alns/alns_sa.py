...
"""
ALNS with Simulated Annealing (SA) acceptance and roulette wheel operator selection.
Destroy: random removal, worst removal.
Repair: greedy insertion.
SA parameters: T_start = -(w * obj0) / ln(0.5), w=0.05, cooling rate c=0.9997.
...
class ALNS_SA:
    def __init__(self, inst: CVRPInstance, selector='roulette'):
        ...
        # SA content
        self.T = None # it depends on the initial sol thats why set to none
        self.w = 0.05
        self.T_start = None
        self.c = 0.9997
    ...
    def solve(self, initial_solution, n_iter=1000, n_seg=100):
        ...
        # for SA
        self.T_start = -(self.w * current_obj) / math.log(0.5) # sol = z'
        self.T = self.T_start
        ...
        for i in range(n_iter):
            ...
            for j in range(n_seg):
                ...
                # cooling T after each destory & repair cycle
                self.T *= self.c
    ...
    def neural_network(self, op_weights, op_type):
      pass
    ...
    def accept(self, current_obj, candidate_obj):
      # it can return one of 3 things: improved (candidate < current) , sa_accept (candidate >= current), rejected
      ...
      if candidate_obj < current_obj:
        ...
        if r < P: # if worse check with prob first
          return 'sa_accepted'
        else:
    ...
    def get_reward(self, candidate_obj, current_obj, best_obj, accepted):
        '''
        then based on accept() it maps
        improved -> sigma1 (if < best else: sigma2)
        sa_accepted -> sigma3
        rejected -> 0
        '''
