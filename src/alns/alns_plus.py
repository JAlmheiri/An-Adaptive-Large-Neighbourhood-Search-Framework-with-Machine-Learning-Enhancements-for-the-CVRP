'''
ALNS+  Adaptive Large Neighborhood Search with additional operators
...
class ALNS_plus:
    def __init__(self, inst: CVRPInstance, selector='roulette'):
        ...
        self.D = [self.random_removal, self.worst_removal, self.shaw_removal] # random index 0, worst 1, shaw 2
    ...
    def solve(self, initial_solution, n_iter=1000, n_seg=100):
        ...
        for i in range(n_iter):
            self.reset_scoresNusage() # reset current score (to 0 at the start of every segment)
            # picking new q for each segment
            q = random.randint(5, 15)

            for j in range(n_seg):

                dest_index, _ = self.select_operator(self.dest_weights, op_type='destroy')
                rep_index,  _ = self.select_operator(self.rep_weights,  op_type='repair')
                ...
                # update regardless of acceptance or rejection
    ...
    def roulette_wheel(self, op_weights):
        ...
        # based on those probs (higher prob -> bigger slice in wheel -> high likelihood of being landed on)
    ...
    def compute_costs4worst(self, sol):
        ...
        costs = []
        # getting neighbors of customer on route + calculating costs based on formula in paper
        for route in sol:
        ...
        costs.sort(reverse=True) # storing with most expensive cust at the start
